...
def send_rest_message(cname, score):
    URL = "http://192.168.0.11:49986/api/v1/resource/sample-json/json"
    headers = {'Content-type': 'application/json'}
    msg = {"camera":"camera1","thing": cname, "score":score}
    print (msg)
    j = json.dumps(msg)
...
def send_mqtt_message(cname, score):
    ...
    topic = "camera1/objects"
    msg = "{}:{}"
    pubClient.publish(topic, msg.format(cname,score))
...
def load_camera():
    ...
    global cap
    cap = cv2.VideoCapture("udpsrc uri=udp://192.168.0.11:5000 ! application/x-rtp,encoding-name=jpeg,payload=26 ! rtpjpegdepay ! jpegdec ! videoconvert ! appsink");
